...
export function MemberPortalWaitlistForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!endpoint) {
      setStatus('error');
      setFormError('Add NEXT_PUBLIC_FORMSPREE_ENDPOINT to enable member portal waitlist submissions.');
      return;
    }
    ...
    formData.set('_subject', 'Diaz Martial Arts member portal waitlist');
    formData.set('source', 'Member portal coming soon page');
    ...
    try {
      ...
      setStatus('success');
      setFieldErrors({});
      setStatusMessage('Thanks. You are on the member portal waitlist.');
      form.reset();
    } catch {
      setStatus('error');
  ...
  return (
    <form id="waitlist" onSubmit={handleSubmit} className="grid gap-3" noValidate>
      <div>
        <label htmlFor="portal-waitlist-name" className="sr-only">
          Full name
        </label>
        <input
          id="portal-waitlist-name"
          name="name"
          required
          autoComplete="name"
      ...
      <div>
        <label htmlFor="portal-waitlist-email" className="sr-only">
          Email address
        </label>
        <input
          id="portal-waitlist-email"
          name="email"
          type="email"
          required
          autoComplete="email"
      ...
      <div>
        <label htmlFor="portal-waitlist-status" className="sr-only">
          Membership status
        </label>
        <select id="portal-waitlist-status" name="message" className={inputClasses} defaultValue="">
          <option value="" disabled>
            Choose one...
          </option>
          <option>I&apos;m a current Diaz Martial Arts member</option>
          <option>I&apos;m interested in becoming a member</option>
          <option>I&apos;m asking for a family member</option>
        </select>
      </div>
