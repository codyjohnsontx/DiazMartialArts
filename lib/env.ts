...
function parseAbsoluteUrl(name: string, value: string): URL {
  try {
    ...
    throw new Error(
      `[env] ${name} must be a full absolute URL including protocol. Example: https://www.diazmartialarts.com`,
    );
  }
}
...
function normaliseConfiguredSiteUrl(value: string): string {
  ...
  if (url.protocol !== 'http:' && url.protocol !== 'https:') {
    throw new Error(
      '[env] NEXT_PUBLIC_SITE_URL must use http: or https:. Example: https://www.diazmartialarts.com',
    );
  }

  if (url.search || url.hash) {
    throw new Error(
      '[env] NEXT_PUBLIC_SITE_URL must be a bare site URL with no query string or fragment. Example: https://www.diazmartialarts.com',
    );
  }
...
function resolveDerivedSiteUrl(): string {
  ...
  throw new Error(
    '[env] Missing NEXT_PUBLIC_SITE_URL. Set it to the public site URL, for example https://www.diazmartialarts.com',
  );
}
