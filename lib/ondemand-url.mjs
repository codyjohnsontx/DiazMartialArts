 ...
 * placeholder exactly like an unset value: `/ondemand`, the site's one member
 * entry point, renders the coming-soon page rather than forwarding to a host
 * that does not exist. A malformed value still throws, so a real typo fails
...
export function resolveOndemandUrl(rawValue) {
  ...
  try {
    ...
    throw new Error(
      '[env] NEXT_PUBLIC_ONDEMAND_URL must be a full absolute URL including protocol. Example: https://diaz-martial-arts.vercel.app',
    );
  }
